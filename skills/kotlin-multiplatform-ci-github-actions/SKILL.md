...
- Confirm the macOS/iOS job stays on the required per-push gate only if iOS
  ...
  `kotlin-multiplatform-roborazzi`, if push-frequency cost is the real problem
...
```bash
bash skills/kotlin-multiplatform-ci-github-actions/scripts/install-act.sh
bash skills/kotlin-multiplatform-ci-github-actions/scripts/run-ci-locally.sh .github/workflows/ci.yml
```
