...
description: >
  ...
  Produces two workflow files: ci.yml (Android tests on Ubuntu, iOS tests on
  macOS, lint, Gradle cache) and release.yml (XCFramework build + upload artifact).
  Assumes AGP 9+ and the project structure from kotlin-multiplatform-feature-scaffold.
license: Apache-2.0
metadata:
...
| File | Trigger | Jobs |
|---|---|---|
| `.github/workflows/ci.yml` | push to `main`, all PRs | `lint`, `test-android`, `test-ios` |
...
1. Open a draft PR — confirm `lint`, `test-android`, `test-ios` all trigger
