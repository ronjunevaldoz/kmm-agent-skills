...
- `kmp-expect-actual` — for `actual` implementations on Android that call the JNI bridge; the `expect` interface keeps the Kotlin engine platform-agnostic
- `kmp-unit-testing` — testing the Kotlin engine class above the JNI layer with `runTest` and fakes
