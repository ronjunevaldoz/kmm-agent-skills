---
name: kotlin-multiplatform-native-authoring
description: >
  ...
  Produces the artifact kotlin-multiplatform-jni-pro's Phase 0 discovery expects to find.
  Does NOT cover bridging Kotlin to native code — that is kotlin-multiplatform-jni-pro's
...
- A 3rd-party C/C++ library already exists and needs a Kotlin bridge — that's
  `kotlin-multiplatform-jni-pro`'s Phase 0 (library-first discovery); this skill's whole
...
- The task is writing the actual JNI/cinterop glue for native code that's already
  built — that's `kotlin-multiplatform-jni-pro`, always the next skill after this one,
...
- The native need is trivial (a handful of pure functions with no state, no build
  ...
  `kotlin-multiplatform-expect-actual` may cover it without a separate C/C++ core at all
...
2. **A stable public C-ABI header before any Kotlin code exists.** `native/include/` is
   ...
   doesn't. This header is exactly what `kotlin-multiplatform-jni-pro`'s Phase 0
...
- a stable header lets Kotlin-side work (this skill's output feeds directly into
  `kotlin-multiplatform-jni-pro`) start before every native feature is finished
...
Rules:
- `include/` is the only directory `kotlin-multiplatform-jni-pro`'s Phase 0 discovery
...
- **iOS / Kotlin-Native**: build `vulkanrenderer` as a static lib, then point a `.def` file
  at `include/` — `kotlin-multiplatform-jni-pro`'s cinterop guidance takes over from here
- **Desktop/JVM**: build the shared lib, load it via `System.loadLibrary("vulkanrenderer")`
  in the JNI bridge `kotlin-multiplatform-jni-pro` writes

## Step 4: Handoff to `kotlin-multiplatform-jni-pro`

Once `native/include/<library>/<library>.h` compiles and its public functions are stable,
that header is the input to `kotlin-multiplatform-jni-pro`'s Phase 0 (library-first
...
- **Treating this skill's output as done and skipping `kotlin-multiplatform-jni-pro`** —
...
- `kotlin-multiplatform-jni-pro` — always the next step after this skill; bridges Kotlin
...
- `kotlin-multiplatform-expect-actual` — for native needs simple enough that a plain
  Kotlin/Native implementation covers it without a separate C/C++ core at all
- `kotlin-multiplatform-library-publishing` — a library with a native core still ships
...
- `kotlin-multiplatform-api-mimicry` — if the native core also needs a mimicked
...
5. the explicit handoff point to `kotlin-multiplatform-jni-pro`
