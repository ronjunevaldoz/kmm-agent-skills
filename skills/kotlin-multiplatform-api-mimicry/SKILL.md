---
name: kotlin-multiplatform-api-mimicry
description: >
...
- The project already depends on the real reference library (Compose Multiplatform,
  ...
  see `kotlin-multiplatform-design-system` for building a design system **on top of**
...
A KMM library wraps a custom Vulkan rendering backend (via `expect`/`actual` +
JNI/cinterop — see `kotlin-multiplatform-jni-pro` and
`kotlin-multiplatform-expect-actual`) and needs a Compose-shaped declarative UI layer on
top of it.
...
- This is almost always a **library** project (see
  `kotlin-multiplatform-library-publishing`), not an app — scaffold accordingly
...
- `kotlin-multiplatform-library-publishing` — this is almost always a library project;
  publish it the same way as any other KMP library
- `kotlin-multiplatform-jni-pro` — if the custom runtime is native (Vulkan, Metal via
  cinterop, etc.), the JNI/cinterop bridge for it lives here
- `kotlin-multiplatform-expect-actual` — platform-specific backend wiring for the
  custom renderer
- `kotlin-multiplatform-design-system` — use this instead when the target runtime
  **is** real Compose Multiplatform; that skill builds a design system on top of the
  real thing rather than mimicking its shape
- `kotlin-multiplatform-compose-slot-api` — slot-lambda patterns for real Compose
  Multiplatform projects; this skill's slot-lambda guidance is the non-Compose analog
- `kotlin-multiplatform-clean-architecture` — layer boundaries still apply inside the
