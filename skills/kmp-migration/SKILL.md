...
description: >
  ...
  toward the kmp-agent-skills architecture. Covers assessment of current state,
...
```bash
# Add detekt.yml with architecture rules (from kmp-clean-architecture)
...
| Skill | Why first |
|---|---|
| `kmp-clean-architecture` | Sets the contract all other migrations must satisfy |
| `kmp-code-quality` | Detekt gates prevent new violations during migration |
| `kmp-feature-scaffold` | Defines the module structure to migrate toward |
| `kmp-project-docs-maintainer` | Documents the target architecture and migration decisions as Phase 2 makes them, not deferred to the end — a migration that stalls mid-way with no written contract is worse than the state it started from |

### Tier 2 — Screen layer (apply per screen as you migrate)

| Skill | When to apply |
|---|---|
| `kmp-mvi` | When migrating a screen from MVVM to MVI |
| `kmp-presenter-module` | When extracting `:presenter` from a monolithic module |
| `kmp-unit-testing` | Immediately after migrating each ViewModel |

### Tier 3 — Infrastructure (after screen layer is stable)

| Skill | When to apply |
|---|---|
| `kmp-network-layer` | When migrating from Retrofit/Volley to Ktor |
| `kmp-repository-pattern` | When splitting data access from ViewModels. If a feature being migrated has no real backend yet (a common mid-migration state — old screen ripped out, new one not API-ready), wire an `InMemory<Feature>Repository` behind the same interface first ("In-memory repository (no backend yet)" section) rather than blocking the migration on backend work. |
| `kmp-dependency-injection` | When migrating from Hilt/manual DI to Koin 4 |
| `kmp-sqldelight-setup` | When migrating from Room to SQLDelight |
| `kmp-android-cli` | Once the Android target builds again after the module-graph migration — verify it builds/runs/deploys from the terminal instead of only re-confirming through Android Studio |
...
| Skill | When to apply |
|---|---|
| `kmp-expect-actual` | When extracting platform-specific code to KMP |
| `kmp-xcframework-spm` | When adding iOS target |
| `kmp-ci-github-actions` | When the architecture is stable enough to gate on CI |
...
- `kmp-clean-architecture` — the contract all migrations must satisfy
- `kmp-mvi` — the target pattern for screen-level migration
- `kmp-feature-scaffold` — the module structure to migrate toward
- `kmp-audit` — run first; `--roadmap` flag gives the adoption plan
- `kmp-unit-testing` — write tests before migrating each component
...
| Date | Change |
|---|---|
| 2026-07-19 | Added `kmp-project-docs-maintainer` to Tier 1 Foundation and `kmp-android-cli` to Tier 3 Infrastructure — real gap: a project on an older kmp-agent-skills release that never adopted these (both became part of the mandatory new-project baseline this same day) had no adoption-order guidance for retrofitting them into an existing migration. |
| 2026-07-11 | Added `/kmp-migrate-to-shadcn` to Tier 4 — full design-system-to-shadcn-compose library swap, distinct from the existing "consolidating hardcoded colors/spacing" design-system row. |
