...
def read_skills_version(project_root: Path) -> str | None:
    skills_file = project_root / ".kmp-skills"
...
def validate_skills_version_pin(project_root: Path) -> list[dict]:
    ...
    if not skills_file.exists():
        return [{
            ...
            "file": ".kmp-skills",
            "line": 0,
            "message": "Missing .kmp-skills version pin. Pin the skills collection to a release tag before running governance.",
        }]
    ...
    if not version:
        return [{
            ...
            "file": ".kmp-skills",
            "line": 0,
            "message": "Malformed .kmp-skills file. Pin the skills collection to a release tag such as v1.25.11.",
        }]
...
def print_report(findings: list[dict], threshold: str, version: str | None) -> None:
    if version:
        ...
        print("KMP Skills governance — no .kmp-skills version file found")
    print()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KMP skills governance check")
