...
:core:designsystem
  ...
  ├── components/    ← skill-owned, update via /kmp-update-design-system
...
> **These are project-owned.** The `/kmp-update-design-system` command will never touch
...
| Components | `components/COMPONENT_PREFIX*.kt` | **Skill** | Run `/kmp-update-design-system` to pull bug fixes; review diff before applying |
| Previews | `previews/COMPONENT_PREFIX*Preview.kt` | **Skill** | Regenerated when component updates |
| Detekt rules | `detekt-rules/` | **Skill** | Pulled via `/kmp-update-design-system`; config in `detekt-design-system.yml` is project-owned |
...
Record the last time `/kmp-fix-design` and `/kmp-audit-design-visual` were run:

| Date | Command | Findings | Action taken |
|---|---|---|---|
| _fill in_ | `/kmp-fix-design` | _N violations_ | _fixed / deferred_ |
| _fill in_ | `/kmp-audit-design-visual` | _N screens reviewed_ | _fixed / deferred_ |
| _fill in_ | `/kmp-record-design-baselines` | _N goldens updated_ | |
