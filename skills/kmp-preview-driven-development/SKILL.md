...
The Screen/Content split (from `kmp-presenter-module`) is the technical prerequisite:
...
See `kmp-roborazzi` for the Gradle plugin wiring.
...
- `kmp-presenter-module` — the Screen/Content split that makes previews stateless
- `kmp-roborazzi` — promotes previews to CI screenshot tests
- `kmp-feature-scaffold` — the Desktop target must be declared in the feature's `:ui` convention plugin
- `kmp-unit-testing` — unit tests for the ViewModel; PDD covers the UI layer
