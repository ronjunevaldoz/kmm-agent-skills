...
`kotlin-multiplatform-shadcn-compose` — don't suggest `App*`/`AppTheme` for it.** Same
rule as that skill's own "never combine" note, stated here too since either skill can be
the one consulted first. `kotlin-multiplatform-audit`'s
...
- bundling multiple `*Variant` sealed types into one `styles/` file — `styles/ButtonStyles.kt` holds exactly `ButtonVariant`, matched 1:1 with `components/AppButton.kt`; `kotlin-multiplatform-audit`'s `_detect_combined_style_file` flags 2+ variant types in one file
...
- `kotlin-multiplatform-audit` — `_detect_combined_component_file`/`_detect_combined_style_file` mechanically check the one-component-per-file and one-component's-variants-per-file conventions above
...
| Date | Change |
|---|---|
| 2026-07-26 | Named the one-variant-type-per-file convention for `styles/` explicitly and backed it with `kotlin-multiplatform-audit`'s new `_detect_combined_style_file` — the same bundling problem as combined component files, one directory over. 1 new anti-pattern. |
