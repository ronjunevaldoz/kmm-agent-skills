...
Rules for the name:
- Must start with `kotlin-multiplatform-` (or `jni-` for native bridge skills)
- Must be kebab-case, all lowercase, no spaces
- Must not already exist: `ls skills/ | grep $ARGUMENTS`
- Must not duplicate an existing skill's concern — check `skills/kotlin-multiplatform-expert/SKILL.md` routing map
...
Create `skills/kotlin-multiplatform-$ARGUMENTS/SKILL.md` with this exact structure:

```markdown
---
name: kotlin-multiplatform-$ARGUMENTS
description: >
...
1. Update the expert skill's routing map (`skills/kotlin-multiplatform-expert/SKILL.md`):
...
```bash
git add skills/kotlin-multiplatform-$ARGUMENTS/ \
