# /kmm-migrate-to-shadcn

Migrates an existing project from the owned `kotlin-multiplatform-design-system`
...
experimental-API risk before touching any code, same as `/kmm-new-project`'s Step 6a
warning gate.
...
This is the same risk `/kmm-new-project` Step 6a warns about, but higher-stakes here —
...
Same vibe-based inference as `/kmm-new-project` Step 6a-ii — infer the app type from the
project's existing README/description/domain, draft a preset/baseColor/accent
recommendation using the table in `kotlin-multiplatform-shadcn-compose`'s "Picking a
...
Load `kotlin-multiplatform-shadcn-compose` for the Gradle setup (Maven coordinate,
...
Same per-file confirmation pattern as `/kmm-fix-design`:
...
Run `/kmm-audit-screenshots` against the updated goldens to catch any visual regression
...
(everything `kotlin-multiplatform-design-system`/`-extended` generated), and remove the
